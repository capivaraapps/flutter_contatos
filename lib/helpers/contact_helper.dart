...
class ContactHelper {
  ...
  Future<int> deleteContact(int id) async {
    Database dbContact = await db;
    return await dbContact.delete(contactsTable, where: "$idColumn = ?", whereArgs: [id]);
  }

  Future<int> deleteAllContacts() async {
    Database dbContact = await db;
    return await dbContact.delete(contactsTable);
  }
  ...
  Future<int> countContacts() async {
    Database dbContact = await db;
    return Sqflite.firstIntValue(await dbContact.rawQuery("SELECT COUNT(*) FROM $contactsTable"));
  }
